...
const Slide = ({ slide, index, current, handleSlideClick }: SlideProps) => {
  ...
  return (
    <div className="[perspective:1200px] [transform-style:preserve-3d]">
      <li
        ref={slideRef}
        className="relative mr-[4vmin] flex h-[70vmin] w-[70vmin] flex-1 flex-col items-center justify-center text-center text-white opacity-100 transition-all duration-300 ease-in-out snap-start first:ml-0"
...
const CarouselControl = ({
  ...
  return (
    <button
      className={cn(
        "mx-2 flex h-10 w-10 items-center justify-center rounded-full border-2 border-transparent bg-neutral-200 transition duration-200 hover:-translate-y-0.5 active:translate-y-0.5 focus:outline-none focus:border-[#6D64F7] dark:bg-neutral-800",
...
export function Carousel({ slides, className }: CarouselProps) {
  ...
  return (
    <div
      className={cn(
        "relative h-[70vmin] max-h-[680px] w-full max-w-none",
        "sm:h-[60vmin] md:h-[55vmin] lg:h-[52vmin]",
        className,
      )}
      aria-labelledby={`carousel-heading-${id}`}
    >
      <ul
        className="absolute left-0 flex justify-start transition-transform duration-1000 ease-in-out [transform-style:preserve-3d] snap-x snap-mandatory"
        style={{
      ...
      <div className="absolute top-[calc(100%+7rem)] sm:top-[calc(100%+14rem)] flex w-full justify-center">
        <CarouselControl
          type="previous"
